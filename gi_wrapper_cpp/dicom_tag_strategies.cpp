...
bool rewriteAllTagsStrategy(int, int, const set <pair<int, int>>)
{
    return true;
}

// None of DICOM tags from source DICOM are rewritten into result DICOM.
bool rewriteNoTagsStrategy(int, int, const set <pair<int, int>>)
{
    return false;
}

// Only non-private DICOM tags from source DICOM are rewritten into result DICOM.
bool rewriteAllNonPrivateTagsStrategy(int tagGroupId, int, const set <pair<int, int>>)
{
...
bool rewriteSpecifiedTagsStrategy(int tagGroupId, int tagTagId, const set <pair<int, int>> tagSet)
{
...
bool omitSpecifiedTagsStrategy(int tagGroupId, int tagTagId, const set <pair<int, int>> tagSet)
{
    pair<int, int> tag = make_pair(tagGroupId, tagTagId);
    return tagSet.find(tag) == tagSet.end();
}
...
void applyDicomTagsRewriteStrategy(DataSet& source, DataSet& destination, int rewriteTagsStrategyId, const set <pair<int, int>> userTags)
{
    ...
    for (DataSet::ConstIterator src = source.Begin(), srcEnd = source.End(); src != srcEnd; ++src)
    {   
        if (rewriteStrategy(src->GetTag().GetGroup(), src->GetTag().GetElement(), userTags))
        {
            // we create new data so we shouldn't rewrite this tag from input dicom
            if (src->GetTag().GetGroup() == DCM_GROUP_PIXEL_DATA) {
                continue;
            }

            destination.Replace(*src);
            DataElement destinationDataElement = destination.GetDataElement(src->GetTag());

            if (VR().GetVRString(src->GetVR()) == "SQ") {
                // In DICOM standard item positions start from 1
                for (int i = 1; i <= src->GetValueAsSQ()->GetNumberOfItems(); i++)
                {
                    applyDicomTagsRewriteStrategy(src->GetValueAsSQ()->GetItem(i).GetNestedDataSet(),
